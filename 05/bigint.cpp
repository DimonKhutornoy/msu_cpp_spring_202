...
{
	bool fl=true;
	if (cnt!=op.cnt || sgn!=op.sgn) return false;
	for (int i=0; i<cnt; i++)
	{
		if(!fl) break;
		fl=(num[i]==op.num[i]);
...
{
	int i=cnt-1;
	if (sgn==true && op.sgn==false) return true;
	if (sgn==false && op.sgn==true) return false;
	if (sgn==false && op.sgn==false)
	{
		if (cnt<op.cnt) return true;
		if (cnt>op.cnt) return false;
		while (i>=0 && num[i]==op.num[i]) i--;
		if (num[i]<op.num[i]) return true;
		return false;
	}
	BigInt x;
	BigInt y;
...
{
	int i=cnt-1;
	if (sgn==true && op.sgn==false) return false;
	if (sgn==false && op.sgn==true) return true;
	if (sgn==false && op.sgn==false)
	{
		if (cnt<op.cnt) return false;
		if (cnt>op.cnt) return true;
